...
const UserForm = ({ open, setOpen, styles }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const CV = uploadCv && (await handleCVUpload(uploadCv));
      const newData = uri ? { ...data, profileUrl: uri, cvUrl: CV } : data;
  ...
  return (
    <>
      <Transition appear show={open ?? false} as={Fragment} >
        <Dialog as='div' className={`relative z-10 ${styles}`} onClose={closeModal}>
          <Transition.Child
            as={Fragment}
...
const UserProfile = () => {
  ...
  return (
    <div className='container mx-auto flex items-center justify-center py-10 px-5'>
        ...
        <div className='w-full py-10'>
          <div className='w-full flex flex-col-reverse md:flex-row gap-8 py-6'>
            <div className='w-full md:w-2/3 flex flex-col gap-3 text-lg text-slate-700 mt-20 md:mt-0'>
              ...
              {((userInfo._id === user._id) || (user.accountType !== "seeker")) && (
              <>
                <p className='font-semibold text-2xl'>User cv</p>
                <span className='text-base text-justify leading-7'>
                  {userInfo?.cvUrl ? <a href={userInfo?.cvUrl} target="_blank" rel="noopener noreferrer" className="font-medium text-blue-600 dark:text-blue-500 hover:underline">View CV</a> : "No CV uploaded"}
                </span>
              </>)}
            </div>
      ...
      <UserForm open={open} setOpen={setOpen} styles="mt-24" />
    </div>
  );
};
